...
$card_data_q = $db->query("SELECT * FROM cards WHERE cardid = '{$getcardid}'",PDO::FETCH_ASSOC);
$card_data_q_query = $card_data_q->fetch(PDO::FETCH_ASSOC);
$cardmoney = $card_data_q_query['money'];
$cardowner = $card_data_q_query['owner'];
...
$yatirilcakmiktar_is_numeric = is_numeric($yatirilcakmiktar);
if($yatirilcakmiktar_is_numeric) {
if($getusermoney < $yatirilcakmiktar) {
    ...
    $newmoney = $getusermoney - $yatirilcakmiktarnew;
    ...
    if($update_user_money_query && $update_card_money_query){
...
<html lang="tr">
  <head>
    ...
    <title>Para Yükle | Nomee6 Bank</title>
    <!-- CSS files -->
