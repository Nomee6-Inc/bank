...
if (isset($_POST['submit'])) {
    if($_POST['secretrenew'] == "on") {
        $newsecretkey = $createappsecret;
    } else {
        $newsecretkey = $secret;
    }
    $getappname = htmlentities($_POST['appname']);
    $getappcallback = $_POST['appcallback'];
  	$newiban = htmlentities($_POST['moneywithdrawiban']);
    if(filter_var($getappcallback, FILTER_VALIDATE_URL) && $getappname) {
  	$sql2 = $db->prepare("UPDATE payment_apps SET
	name = :new_appname,
    callback = :new_callback,
    secret = :new_secret,
    money_withdraw_iban = :new_iban
	WHERE client_id = :clientid");
	$query2 = $sql2->execute(array(
	     "new_appname" => "$getappname",
      	 "new_callback" => "$getappcallback",
    ...
    if($query2) {
      ...
      echo "<script>alert(\"Lütfen geçerli bir URL girin!\")</script>";
   }
}  
}
...
<html lang="tr">
  <head>
    ...
    <title>Uygulama Yönet | NOMEE6 Bank</title>
    <!-- CSS files -->
                            ...
                            <div class="card">
                              <div class="card-body">
                                <div class="mb-3">
                                  ...
                                  <input type="text" name="appname" class="form-control" autocomplete="off" value="<?php echo $appname; ?>" required/>
                                </div>
                                <div class="mb-3">
                                  <div class="form-label">Uygulama Callback adresi</div>
                                  <input type="url" name="appcallback" class="form-control" value="<?php echo $callback; ?>" required>
                                </div>
                                <div class="mb-3">
                                  <div class="form-label">Paranın çekileceği IBAN</div>
                                  <input type="text" name="moneywithdrawiban" class="form-control" value="<?php echo $money_withdraw_iban; ?>" required>
                                </div>
                                <div class="mb-3">
                                  <div class="form-label">Secret Key:</div>
                                  <div class="form-label"><?php echo $secret; ?></div>
                                  <label class="form-check form-check-inline">
                                    <input name="secretrenew" class="form-check-input" type="checkbox">
                                    <span class="form-check-label">Secret Key Yenile</span>
                                  </label>
                                </div>
                    <div class="mt-2">
                        <button type="submit" name="submit" class="btn btn-primary w-100">Kaydet</button>
                      </div>
                    </div>
                  </div>
                </div>
