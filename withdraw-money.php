...
if($cardmoney < $cekilcekmiktar) {
    ...
    if($getusername == $cardowner) {
        ...
        $newmoney = $money + $cekilcekmiktar - 1;
        $sql7 = "UPDATE cards SET money = '$newbankmiktar' WHERE cardid = '$cardid'";
        $run_query = mysqli_query($conn, $sql7);
	    $sql8 = "UPDATE users SET money = '$newmoney' WHERE baccid = '$getaccid'";
        $run_query8 = mysqli_query($conn, $sql8);
        if($run_query && $run_query8){
            echo "Başarıyla ₺$cekilcekmiktar tutarında para çektiniz.";
            header("Location: panel.php");
        } else {
